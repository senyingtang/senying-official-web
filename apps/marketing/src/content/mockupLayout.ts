...
export const MOCKUP_LAYOUTS: MockupPageLayout[] = [
  ...
  },
  {
    ...
    // Phase 3.0：這一頁從「方案示意」升級為實際結帳流程（購物車 → 客戶資料 → 付款方式 → 訂單摘要）
    sections: [
      { key: 'hero', title: 'Hero 區塊', spec: '確認方案，完成結帳；明確標示目前只有本機 Sandbox 模擬付款' },
      { key: 'cart-review', title: '購物車內容', spec: '購物車品項與數量；空車時顯示可自助購買的方案入口' },
      { key: 'customer', title: '客戶資料', spec: '姓名 / Email / 電話 / 公司 / 統編 / 備註 + 隱私權同意；不收集卡號或 CVV' },
      { key: 'payment-methods', title: '付款方式', spec: '只顯示資料庫中已啟用的付款方式；模擬付款必須標示' },
      { key: 'order-summary', title: '訂單摘要', spec: '件數、小計、應付金額與送出按鈕；桌機 sticky、手機可折疊' },
