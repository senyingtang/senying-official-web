-- =====================================================================
-- 0002_admin_and_cms_baseline.sql
-- v1.0 `0002_core_schema.sql` 全表沿用（表名、欄位名、約束不變），作為森映官方 CMS 基底。
-- v2.0 差異（向後相容、只增不減）：
--   1. audit_logs 新增 actor_type / workspace_id / metadata（第三方可追溯客戶端操作）。
--   2. conversion_events 新增 site_project_id（未來客戶網站轉換事件共用）。
--   3. 新增 customer_profiles（客戶端基本資料，非後台帳號）。
-- 若既有環境已套用 v1.0，本檔 create table if not exists + alter table add column if not exists 可安全重跑。
-- =====================================================================
begin;
...
alter table public.blog_post_tags add column if not exists created_at timestamptz not null default now();

-- ---------------------------------------------------------------------
-- 產品與服務（森映官網展示用；可販售方案另見 commerce_products）
-- ---------------------------------------------------------------------
...
create table if not exists public.case_assets(
  ...
  asset_role text not null default 'gallery', sort_order int not null default 0,
  created_at timestamptz not null default now(),
  primary key(case_id,asset_id)
);
alter table public.case_assets add column if not exists created_at timestamptz not null default now();
...
alter table public.case_services add column if not exists created_at timestamptz not null default now();
...
alter table public.case_products add column if not exists created_at timestamptz not null default now();
...
-- audit_logs：append-only（僅 created_at，無 updated_at；0012 以 trigger 禁止 update/delete）
...
alter table public.audit_logs add column if not exists actor_type text not null default 'admin';
alter table public.audit_logs add column if not exists workspace_id uuid; -- FK 於 0005 補上
alter table public.audit_logs add column if not exists metadata jsonb not null default '{}'::jsonb;
do $$ begin
  alter table public.audit_logs add constraint audit_logs_actor_type_check check (actor_type in ('admin','customer','system','webhook'));
exception when duplicate_object then null; end $$;

-- ---------------------------------------------------------------------
-- v2.0 新增：客戶基本資料（客戶端帳號，與 admin_profiles 分離）
-- ---------------------------------------------------------------------
create table if not exists public.customer_profiles(
  ...
  tax_id text check (tax_id is null or tax_id ~ '^[0-9]{8}$'), -- 統一編號（選填）
